...
export class MessageController {
  ...
  @Post('schedule')
  @RequireRole(ApiKeyRole.OPERATOR)
  @ApiOperation({ summary: 'Schedule a message or poll to be sent later' })
  @ApiParam({ name: 'sessionId', description: 'Session ID' })
  @ApiResponse({ status: 201, description: 'Message scheduled', type: ScheduledMessageResponseDto })
  async scheduleMessage(
    @Param('sessionId') sessionId: string,
    @Body() dto: ScheduleMessageDto,
  ): Promise<ScheduledMessageResponseDto> {
    return this.scheduledMessageService.schedule(sessionId, dto);
  }
  ...
  @ApiParam({ name: 'sessionId', description: 'Session ID' })
  @ApiResponse({ status: 200, description: 'Scheduled messages', type: [ScheduledMessageResponseDto] })
  async listScheduled(@Param('sessionId') sessionId: string): Promise<ScheduledMessageResponseDto[]> {
    return this.scheduledMessageService.list(sessionId);
  }
  ...
  @ApiResponse({ status: 200, description: 'Scheduled message cancelled', type: ScheduledMessageResponseDto })
  async cancelScheduled(
    @Param('sessionId') sessionId: string,
    @Param('id') id: string,
  ): Promise<ScheduledMessageResponseDto> {
    return this.scheduledMessageService.cancel(sessionId, id);
  }
