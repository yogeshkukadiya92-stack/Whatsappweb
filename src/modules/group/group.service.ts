...
export class GroupService {
  ...
  async exportGroupParticipantsCsv(sessionId: string, groupId: string): Promise<string> {
    const group = await this.getGroupInfo(sessionId, groupId);
    const headers = ['Group Name', 'Participant Name', 'Mobile Number', 'WhatsApp ID', 'Role'];
    const rows = (group.participants || []).map(p => {
      const role = p.isSuperAdmin ? 'Super Admin' : p.isAdmin ? 'Admin' : 'Member';
      const name = (p.name || '').replace(/"/g, '""');
      const number = p.number || p.id.replace('@c.us', '');
      return [
  ...
  async exportAllGroupsParticipantsCsv(sessionId: string): Promise<string> {
    const engine = this.getEngine(sessionId);
    const groups = await engine.getGroups();
    const headers = ['Group ID', 'Group Name', 'Participant Name', 'Mobile Number', 'WhatsApp ID', 'Role'];
    const allRows: string[] = [];

    for (const g of groups) {
      try {
        const info = await engine.getGroupInfo(g.id);
        ...
        // Skip group if engine call times out or throws
      }
    }
