...
import { Send, CheckCircle, XCircle, Loader2, Upload, X, Plus, Clock } from 'lucide-react';
import {
  messageApi,
  contactApi,
...
type SchedulableMessageType = Exclude<(typeof messageTypes)[number], 'bulk' | 'forward'>;
...
export function MessageTester() {
  ...
  const canScheduleType = messageType !== 'bulk' && messageType !== 'forward';
  ...
  const isSendDisabled =
    ...
    !formValid ||
    !scheduleValid ||
    (scheduleEnabled && !canScheduleType) ||
    (scheduleEnabled && !!mediaFile) ||
  ...
  const buildSchedulePayload = (): Record<string, unknown> => {
    switch (messageType) {
      ...
      case 'image':
      case 'video':
        return { url: mediaUrl, ...(content ? { caption: content } : {}) };
      case 'audio':
      case 'sticker':
        return { url: mediaUrl };
      case 'document':
        return { url: mediaUrl, ...(content ? { filename: content } : {}) };
      case 'location':
        return {
          latitude: lat,
          longitude: lng,
      ...
      case 'contact':
        return { contactName: contactName.trim(), contactNumber: contactNumber.trim() };
      case 'poll':
        return {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (scheduleEnabled) {
        const scheduledType = messageType as SchedulableMessageType;
        const scheduled = await messageApi.schedule(session, {
          chatId,
          type: scheduledType,
      ...
      <div className="tester-panels">
        <div className="compose-panel">
          ...
          {canScheduleType && (
            <div className="form-group schedule-box">
              <label className="checkbox-label">
                ...
                />
                {t('messageTester.schedule.enable')}
              </label>
              {scheduleEnabled && (
                <>
                  <label htmlFor="mt-19">{t('messageTester.schedule.when')}</label>
                  <input
                    id="mt-19"
                    type="datetime-local"
                    value={scheduledAt}
                  ...
                  <span className="hint">
                    {mediaFile
                      ? t('messageTester.schedule.mediaFileHint')
                      : scheduleValid
                        ? t('messageTester.schedule.hint')
                        : t('messageTester.schedule.futureHint')}
                  </span>
                </>
              )}
            </div>
          ...
          <button className="send-btn" onClick={handleSend} disabled={isSendDisabled}>
            {isLoading ? <Loader2 className="animate-spin" size={18} /> : scheduleEnabled ? <Clock size={18} /> : <Send size={18} />}
            {isLoading
              ? t('messageTester.sending')
              : canWrite
                ? scheduleEnabled
                  ? t('messageTester.schedule.button')
            ...
            <>
              ...
              <div className="response-details">
                ...
                {response.scheduledId && (
                  <div className="detail-row">
                    <span className="detail-label">{t('messageTester.response.scheduledId')}</span>
                    <span className="detail-value mono">{response.scheduledId}</span>
                  </div>
                )}
                {response.error && (
                  <div className="detail-row">
