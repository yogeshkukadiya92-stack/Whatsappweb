...
export const sessionApi = {
  ...
  getGroups: (id: string) =>
    request<{ id: string; name: string; linkedParentJID?: string | null; timestamp?: number }[]>(
      `/sessions/${id}/groups`,
    ),
...
// =============================================================================
// AI Bot API & Lead Flows API
// =============================================================================

export interface AiBotConfigView {
  id: string;
  sessionId: string;
  enabled: boolean;
...
export const aiBotApi = {
  ...
  updateConfig: (sessionId: string, body: UpdateAiBotConfigInput) =>
    request<AiBotConfigView>(`/sessions/${sessionId}/ai-bot`, {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  ...
  extractDocument: (sessionId: string, filename: string, contentBase64: string, mimeType?: string) =>
    request<ExtractedDocumentResponse>(`/sessions/${sessionId}/ai-bot/extract-document`, {
      method: 'POST',
...
export const leadFlowsApi = {
  listFlows: (sessionId: string) => request<LeadFlow[]>(`/sessions/${sessionId}/lead-flows`),
  createFlow: (sessionId: string, body: CreateLeadFlowInput) =>
    request<LeadFlow>(`/sessions/${sessionId}/lead-flows`, {
      method: 'POST',
      body: JSON.stringify(body),
    }),
  updateFlow: (sessionId: string, id: string, body: Partial<CreateLeadFlowInput>) =>
    request<LeadFlow>(`/sessions/${sessionId}/lead-flows/${id}`, {
      method: 'PUT',
      body: JSON.stringify(body),
    }),
  deleteFlow: (sessionId: string, id: string) =>
    request<void>(`/sessions/${sessionId}/lead-flows/${id}`, { method: 'DELETE' }),

  listLeads: (sessionId: string) => request<LeadEntry[]>(`/sessions/${sessionId}/leads`),
  deleteLead: (sessionId: string, id: string) =>
    request<void>(`/sessions/${sessionId}/leads/${id}`, { method: 'DELETE' }),
  exportCsvUrl: (sessionId: string) => `${API_BASE_URL}/sessions/${sessionId}/leads-export/csv`,
};

// =============================================================================
// Group Contacts / Export API
// =============================================================================
