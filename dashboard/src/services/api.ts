...
export interface ScheduleMessagePayload {
  chatId: string;
  type: ScheduledMessageResponse['type'];
...
export const messageApi = {
  ...
  schedule: (sessionId: string, data: ScheduleMessagePayload) =>
    request<ScheduledMessageResponse>(`/sessions/${sessionId}/messages/schedule`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  getScheduled: (sessionId: string) =>
    request<ScheduledMessageResponse[]>(`/sessions/${sessionId}/messages/scheduled`),
  cancelScheduled: (sessionId: string, id: string) =>
    request<ScheduledMessageResponse>(`/sessions/${sessionId}/messages/scheduled/${encodeURIComponent(id)}/cancel`, {
      method: 'POST',
    }),
